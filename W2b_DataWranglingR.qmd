...
# Option 2:
testdata[,"a"]
testdata[, 1]  # index the first column (better practice to use the name!)
...
Work in small groups (break-out sessions):
...
2.  Change the code to other female names and run the code again.
3.  Change the code to male names and change `sex=="F"` to `sex=="M"`.
...
If you now run the summarize() code from before (but with `group_dat` as input), you will get not the total number of observations, but the observations per name!
...
In a Breakout-session, work in small groups on the assignments (Activity 1 - Activity 8) in this chapter: <https://psyteachr.github.io/quant-fun-v2/data-wrangling-2.html>

## Solutions

We'll use data from a [paper](https://journals.sagepub.com/doi/full/10.1177/0956797617730892) that investigates whether the ability to perform and action influences perception. In particular, the authors wondered whether participants who played Pong would perceive the ball to move faster when they have a small paddle.
...
3.  Make sure you have downloaded the data into your working directory.
...
How do we need to group and summarize the data to get a sum score per person? (Ignoring the reverse coding for now!)
